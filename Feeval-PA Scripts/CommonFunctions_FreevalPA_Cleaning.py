...
def GetVariableSummary(FileNm, Features=["CUR_AADT"]):
    ...
    for Feature in Features:
        ...
        ReturnDat2 = (
            dat[["Name", Feature]].groupby(["Name", Feature], as_index=False).size()
        )
        ReturnDat2 = pd.DataFrame(ReturnDat2).reset_index()
        ReturnDat2.columns = ["Name", Feature, "ObsFreq"]
        ReturnDat2.loc[:, "UniqNo"] = np.hstack(
            ReturnDat2.groupby(["Name"])[Feature]
            .apply(lambda x: np.arange(1, len(x) + 1))
            .values
        )
        ReturnDat2.loc[:, "FreevalSeg"] = (
            ReturnDat2.Name - np.min(ReturnDat2.Name)
        ).apply(lambda x: "Seg{}".format(x + 1))
...
def GetProbData(data, feature, FiName):
    ...
    if np.max(temp) >= 2:
        ...
        ProbRows.loc[:, "NumDuplicates"] = ProbRows.loc[:, feature].apply(
            lambda x: len(x)
        )
...
def CleanAADT_1stLevel(TempData):
    ...
    Tp2 = Tp2.merge(
        TempData,
        left_on=["Name", "CUR_AADT"],
        right_on=["Name", "CUR_AADT"],
        how="left",
    )
...
def CleanCityCode_1stLevel(TempData, feature):
    ...
    """
    Tp2 = (
        TempData.groupby(["Name"]).apply(CleanCityCode_2ndLevel, feature).reset_index()
    )
    Tp2.rename(columns={0: feature}, inplace=True)
    Tp2[feature] = Tp2[feature].fillna(method="bfill")
    Tp2 = Tp2.merge(
        TempData, left_on=["Name", feature], right_on=["Name", feature], how="left"
    )
    Tp2.sort_values("Name")
    Tp2.reset_index(inplace=True, drop=True)
    Tp2.UniqNo = 1
...
def Clean_Tot_Width_1stLevel(TempData, feature):
    ...
    """
    Tp2 = (
        TempData.groupby(["Name", "FreevalSeg"])
        .apply(CleanTotWidth_2ndLevel, feature)
        .reset_index()
    )
    Tp2.rename(columns={0: feature}, inplace=True)
    Tp2[feature] = Tp2[feature].fillna(method="bfill")
    Tp2 = Tp2.merge(
        TempData,
        left_on=["Name", "FreevalSeg", feature],
        right_on=["Name", "FreevalSeg", feature],
        how="left",
    )
...
def CleanDivsrType_1stLevel(TempData, feature):
    ...
    """
    Tp2 = (
        TempData.groupby(["Name"]).apply(CleanDivsrType_2ndLevel, feature).reset_index()
    )
    Tp2.rename(columns={0: feature}, inplace=True)
    Tp2[feature] = Tp2[feature].fillna(method="bfill")
    Tp2 = Tp2.merge(
...
def PlotlyDebugFigs(
    Dat_Plot, MaxDuplicates, feature, SheetNm, OutPath="ProcessedData/Fig/Junk/"
):
    """
    ...
    fig = go.Figure(
        data=[
            go.Bar(
                name="Duplicate {} {} and Freq".format(i, feature),
                x=DuplicateData[i]["FreevalSeg"],
                y=DuplicateData[i][feature],
                text=DuplicateData[i]["ObsFreq"],
            )
            for i in range(1, MaxDuplicates + 1)
        ]
    )
...
def PlotlyDebugFigs_2(
    ...
    feature,
    SheetNm,
    OutPath="ProcessedData/Fig/Junk/",
):
    """
    ...
    for grp, data in enumerate([Dat_Plot_Pre, Dat_Plot_Post]):
        if grp == 0:
            ...
            for UnqNo, Label in enumerate(
                [
                    "Duplicate 1 {} and Freq".format(feature),
                    "Duplicate 2 {} and Freq".format(feature),
                    "Duplicate 3 {} and Freq".format(feature),
                ]
            ):
