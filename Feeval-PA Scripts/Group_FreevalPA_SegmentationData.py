...
def GetAADTsummary(FileNm):
    ...
    ReturnDat2 = (
        dat[["Name", "CUR_AADT"]].groupby(["Name", "CUR_AADT"], as_index=False).size()
    )
    ReturnDat2 = pd.DataFrame(ReturnDat2).reset_index()
    ReturnDat2.columns = ["Name", "CUR_AADT", "ObsFreq"]
    ReturnDat2.loc[:, "UniqNo"] = np.hstack(
        ReturnDat2.groupby(["Name"])["CUR_AADT"]
        .apply(lambda x: np.arange(1, len(x) + 1))
        .values
    )
    ReturnDat2.loc[:, "FreevalSeg"] = (ReturnDat2.Name - np.min(ReturnDat2.Name)).apply(
        lambda x: "Seg{}".format(x + 1)
    )
...
def PlotlyDebugFigs(Dat_Plot, SheetNm, OutPath="ProcessedData/Fig/Junk/"):
    ...
    fig = go.Figure(
        data=[
            go.Bar(
                name="1st AADT and Freq",
                x=dat_1stObs["FreevalSeg"],
                y=dat_1stObs["CUR_AADT"],
                text=dat_1stObs["ObsFreq"],
            ),
            go.Bar(
                name="2nd AADT and Freq",
                x=dat_2ndObs["FreevalSeg"],
                y=dat_2ndObs["CUR_AADT"],
                text=dat_2ndObs["ObsFreq"],
            ),
            go.Bar(
                name="3rd AADT and Freq",
                x=dat_3rdObs["FreevalSeg"],
                y=dat_3rdObs["CUR_AADT"],
                text=dat_3rdObs["ObsFreq"],
            ),
        ]
    )
...
def PlotlyDebugFigs_2(
    Dat_Plot_Pre, Dat_Plot_Post, SheetNm, OutPath="ProcessedData/Fig/Junk/"
):
    """
    Draw plotly figures based on Dat_Plot
    """
    ...
    for grp, data in enumerate([Dat_Plot_Pre, Dat_Plot_Post]):
        if grp == 0:
            ...
            Legend1 = "After"
        for UnqNo, Label in enumerate(
            ["1st AADT and Freq", "2nd AADT and Freq", "3rd AADT and Freq"]
        ):
            dat_sub = data[data.UniqNo == UnqNo + 1]
            fig.add_trace(
                go.Bar(
                    name=Label,
                    x=dat_sub["FreevalSeg"],
                    y=dat_sub["CUR_AADT"],
                    text=dat_sub["ObsFreq"],
                    legendgroup=Legend1,
                ),
                row=grp + 1,
                col=1,
...
for _, row in FileData.iterrows():
    ...
    if np.max(temp) >= 3:
        RetDict["Ret1"].loc[:, "NumVal"] = (
            RetDict["Ret1"].CUR_AADT.apply(lambda x: len(x)).values
        )
        RetDict["Ret1"].reset_index(inplace=True)
        ProbRows = RetDict["Ret1"].loc[
            (RetDict["Ret1"].NumVal >= 3), ["Name", "CUR_AADT"]
        ]
...
Tp2.merge(TempData, left_on=["Name", "AADT"], right_on=["Name", "CUR_AADT"], how="left")
...
for _, row in FileData.iterrows():
    ...
    Tp2.rename(columns={0: "AADT"}, inplace=True)
    Tp2 = Tp2.merge(
        TempData, left_on=["Name", "AADT"], right_on=["Name", "CUR_AADT"], how="left"
    )
    ...
    mask = ((Tp2.DeltaPrevObs > 20000) & (Tp2.DeltaNextObs > 20000)) | (
        (Tp2.DeltaPrevObs < -20000) & (Tp2.DeltaNextObs < -20000)
    )
...
AADT_SumDat.loc[:, "DiffinDuplicates"] = AADT_SumDat.CUR_AADT.apply(
    lambda x: abs(x[1] - x[0]) if len(x) > 1 else 0
)

AADT_SumDat2.reset_index(inplace=True)
AADT_SumDat2.loc[:, "UniqNo"] = np.hstack(
    AADT_SumDat2.groupby(["Name"])["CUR_AADT"]
    .apply(lambda x: np.arange(1, len(x) + 1))
    .values
)
