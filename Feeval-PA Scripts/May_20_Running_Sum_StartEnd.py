...
for path in PathList:
    dat = pd.read_excel(
        os.path.join(OuterDir, path),
        converters={"FCOUNTY": str, "FROUTE": str, "FSEG": str, "Concat": str},
    )
    ...
    dat.loc[:, "RunSumStart"] = (
        dat.groupby(["FCOUNTY", "FROUTE"])["RunningSum"].shift().fillna(0)
    )
...
NLP_dat = NLP_dat[
    ["CTY_CODE", "ST_RT_NO", "NLF_CNTL_B", "NLF_CNTL_E", "NLF_ID", "SEG_NO"]
]
...
NLP_dat.loc[:, "NLPSegBins"] = pd.IntervalIndex.from_arrays(
    NLP_dat.NLF_CNTL_B, NLP_dat.NLF_CNTL_E, "left"
)
...
for name, group in FinDatGrouped:
    try:
        ...
        print(name)
        group.loc[:, "NLPBinRunStart"] = pd.cut(
            group.RunSumStart, pd.IntervalIndex(NLP_1.NLPSegBins)
        )
        group.loc[:, "NLPBinRunEnd"] = pd.cut(
            group.RunSumEnd, pd.IntervalIndex(NLP_1.NLPSegBins)
        )
...
NLPbinsDat = NLPbinsDat[
    ["FCOUNTY", "FROUTE", "FSEG", "FOFFSET", "NLPBinRunStart", "NLPBinRunEnd"]
]

FinDat2 = FinDat.merge(
    NLPbinsDat, on=["FCOUNTY", "FROUTE", "FSEG", "FOFFSET"], how="left"
)
...
FinDat2.to_csv(
    os.path.join(OuterDir, "TestingNLP_ID_Merge_Part1Part2RunningSumStartEndTags.csv")
)
