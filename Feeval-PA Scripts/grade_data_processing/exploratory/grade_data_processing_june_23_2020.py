...
grade_gdf = read_data(
    filename_gdf=path_to_grade_data_file, layer_gdf="SpatialJoin_GradeDataFINAL"
)
...
grade_gdf_asc_sort.loc[:, "foffset_diff"] = grade_gdf_asc_sort.groupby(
    ["name", "seg_no"]
).c_offset_b.diff()

grade_gdf_asc_sort.loc[:, "foffset_diff_max"] = grade_gdf_asc_sort.groupby(
    ["name"]
).foffset_diff.transform(lambda x: x.abs().max())

grade_gdf_asc_sort.loc[:, "c_offset_b_diff"] = grade_gdf_asc_sort.groupby(
    ["name"]
).c_offset_b.diff()

grade_gdf_asc_sort.loc[:, "c_offset_b_diff_max"] = grade_gdf_asc_sort.groupby(
    ["name"]
).foffset_diff.transform(lambda x: x.abs().max())

grade_gdf_asc_sort.loc[:, "count_rows"] = grade_gdf_asc_sort.groupby(
    ["name"]
).c_offset_b_diff.transform(lambda x: x.count())
...
for freeval_name in grade_df_debug_1.name.unique():
    name_feature_grp[freeval_name] = folium.FeatureGroup(name=freeval_name)
