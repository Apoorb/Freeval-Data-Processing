...
class GradeClean:
        ...
        correct_sort_df = (
            self.grade_df_asc_or_desc
                ...
                .sort_values(["name", "cty_code_num", "fseg", "foffset"],
                             ascending=[True]
                                       + self
                             .sort_order_ne_sw[self.grade_df_name])
                .reset_index(drop=True)
        )
        correct_sort_df_test_df = self.create_df_test_sort(
            correct_sort_df=correct_sort_df)
        self.correct_sort_df = correct_sort_df
        self.correct_sort_df_test_df = correct_sort_df_test_df
    ...
    def fix_sort_order(self):
        ...
        incorrect_sort_df_fix_ord = (
            ...
            x.groupby("name").cty_code_count.transform(sum),
                    freeval_seg_jumps=lambda df:
                    df.name.diff().ge(2).cumsum(),
                    f_shift_cty_code=lambda df:
                    df.groupby("freeval_seg_jumps")
                    .cty_code.shift().fillna(df.cty_code),
                    b_shift_cty_code=lambda df:
                    df.groupby("freeval_seg_jumps")
    ...
    def create_df_test_sort(self, correct_sort_df):
...
if __name__ == "__main__":
    ...
    asc_grade_obj = gradepr.GradeClean(
