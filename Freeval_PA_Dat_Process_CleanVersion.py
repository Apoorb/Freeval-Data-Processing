...
def RemoveDuplicatedRows_GetCorLen(data_Comp, dir1 = 'N'):
    ...
    data.sort_values(["FCOUNTY","FROUTE","FSEG","FOFFSET"], inplace =True, ascending = SortOrderDict[dir1])
    # See What rows are duplicated before deletion
    Duplicated_dat = data[data.duplicated(subset=["FCOUNTY","FROUTE","FSEG","FOFFSET"], keep=False)]
    # See Number of rows we are going to delete
    RowsThatNeedDeletion = sum(data.duplicated(["FCOUNTY","FROUTE","FSEG","FOFFSET"]))
    ...
    dat_Uniq.sort_values(["FCOUNTY","FROUTE","FSEG","FOFFSET"], inplace =True, ascending = SortOrderDict[dir1])
    dat_Uniq.reset_index(inplace = True, drop =True)
    coeff_dict = {"N" : -1, "S": 1} # Based on whether offsets are in ascending or descending order
...
Dat_Check_N = Dat_Check_N_a.merge(Dat_Check_N_b, left_on = ls_0, right_on = ls_0, how = "inner")
...
Dat_Check_S = Dat_Check_S_a.merge(Dat_Check_S_b, left_on = ls_0, right_on = ls_0, how = "inner")
...
Dat_Check_N_1.loc[:,'RunningSum']= Dat_Check_N_1.groupby(["FCOUNTY","FROUTE","FSEG"])['FLen_Cor'].cumsum()
...
Dat_Check_N_1_1 = Dat_Check_N_1_a.merge(Dat_Check_N_1_b, left_on = ls, right_on = ls, how = "inner")
...
Dat_Check_S_1_1 = Dat_Check_S_1_a.merge(Dat_Check_S_1_b, left_on = ls, right_on = ls, how = "inner")
Dat_Check_S_1_1.loc[:,'SanityCheck'] = Dat_Check_S_1_1.RunningSum - Dat_Check_S_1_1.FOFFSET
sum(Dat_Check_S_1_1.loc[:,'SanityCheck'] >=0)
Dat_Check_N_1_1.to_csv("ProcessedData\\AuxDataCode\\ErrorCheck_2_North.csv",index= False)
Dat_Check_S_1_1.to_csv("ProcessedData\\AuxDataCode\\ErrorCheck_2_South.csv",index= False)
# Sanity Check 2 
#******************************************************************************************
#Dat_S_Check.loc[:,'dir'] = 'S'
ls = ["FCOUNTY","FROUTE","FSEG"]
Dat_Check_S_1_1.FSEG = Dat_Check_S_1_1.FSEG -1 
Dat_Check_S_1_1.rename(columns = {'RunningSum' : "RunningSum_S",'FOFFSET' : "FOFFSET_S"},inplace = True)

#Dat_N_Check.loc[:,'dir'] = 'N'
Dat_check_1 = Dat_Check_N_1_1.merge(Dat_Check_S_1_1,left_on = ls, right_on = ls, how = "inner")
