...
# No-Build
PathKeyValFi = os.path.join(
    os.path.expanduser("~"),
    "OneDrive - Kittelson & Associates, Inc",
    "Documents",
    "23746 - I-83 TSMO",
    "AADT-I-83-Build-Revised---PM.xlsx",
)

# Get the Key value pair for NB TMS files
NB_KeyVal = pd.read_excel(
    PathKeyValFi, "SB-NB-KeyValue", skiprows=2, usecols="C:E", nrows=16
)
...
for indx, row in NB_KeyVal.iterrows():
    if indx == 0:
        NBDat = GetVolData(
            row["Segment"], row["TMS_File_Name"], row["ADT"], Debug=False
        )
    else:
        TempDat = GetVolData(
            row["Segment"], row["TMS_File_Name"], row["ADT"], Debug=False
...
SB_KeyVal = pd.read_excel(
    PathKeyValFi, "SB-NB-KeyValue", skiprows=23, usecols="C:E", nrows=20
)
SBDat = pd.DataFrame()
for indx, row in SB_KeyVal.iterrows():
    if indx == 0:
        SBDat = GetVolData(
            ...
            row["ADT"],
            DirNBorSB="SB-TMS",
            PatternFNm="^(South|West) *Lane *[0-9] *Volume$",
            Debug=False,
        )
    else:
        TempDat = GetVolData(
