...
os.chdir(
    r"C:\Users\abibeka\OneDrive - Kittelson & Associates, Inc\Documents\MassDOT\Speeds"
)
...
Data_del_len = (
    ...
    - Dat[
        ~(
            (Dat.measurement_tstamp >= pd.datetime(2018, 11, 22))
            & (Dat.measurement_tstamp < pd.datetime(2018, 11, 27))
        )
    ].shape[0]
)
...
Dat4 = Dat2.copy()
Dat4 = (
    Dat4.groupby(["direction", "Hr", "Min"])["Friday", "Saturday", "Sunday", "Monday"]
    .sum()
    .reset_index()
)
...
mux = pd.MultiIndex.from_product(
    [
        ...
        list(All_Routes_Nm.values()) + ["Vsep", "Total_TT"],
    ],
    names=["Dir", "Day", "Seg"],
)
