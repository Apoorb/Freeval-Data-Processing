...
os.chdir(
    r"C:\Users\abibeka\OneDrive - Kittelson & Associates, Inc\Documents\MassDOT\Freeval\1-Bridge-Closed-Per-Weekend"
)
...
for i, data in zip([0, 1], [EB, WB]):
    ...
    theta, r = np.meshgrid(
        np.linspace(0, 2 * np.pi, len(data) + 1), np.arange(len(data.columns) + 1)
    )
...
def PlotRadials(data=WB, name="WB"):
    #
    # Plot the diff days on diff subplots
    fig_WB, axs_WB = plt.subplots(
        2, 2, figsize=(4.5, 4), subplot_kw=dict(projection="polar")
    )
    pc_s = []
    for ax, i in zip(axs_WB.flatten(), [0, 1, 2, 3]):
        data1 = pd.DataFrame(data.iloc[:, i])
        theta, r = np.meshgrid(
            np.linspace(0, 2 * np.pi, len(data1) + 1), np.arange(len(data1.columns) + 1)
        )
