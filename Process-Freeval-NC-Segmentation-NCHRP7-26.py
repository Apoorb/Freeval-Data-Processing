...
import os 
import re
os.chdir(r'C:\Users\abibeka\OneDrive - Kittelson & Associates, Inc\Documents\NCHRP7-26\Data\SegDatabaseNC')
...
Dat = pd.read_csv('full_nc_database.csv',index_col=0)
Dat.columns
test = Dat.head()

Dat.loc[:,"General Purpose Segment Type"].value_counts()
Dat.loc[:,"Segment Length (ft)"].describe()
Dat.loc[:,"# of Lanes: Mainline"].value_counts()
Dat.loc[:,"Acc/Dec Lane Length (ft)"].describe()
Dat.loc[:,"ONR Side"].value_counts()
Dat.loc[:,"# Lanes: ONR"].value_counts()
Dat.loc[:,"OFR Side"].value_counts()
Dat.loc[:,"# Lanes: OFR"].value_counts()

Dat.rename(columns = {"General Purpose Segment Type":"segType", "Segment Length (ft)": "segLenFt", "# of Lanes: Mainline":"segNumLanes",
                      "Acc/Dec Lane Length (ft)": "segAccDecLen", "ONR Side": "segOnrSide", "# Lanes: ONR": "segOnrNumLanes",
                      "OFR Side": "segOfrSide", "# Lanes: OFR" : "segOfrNumLanes"},inplace=True)
...
Dat.loc[:,"Lat"]  = Dat.Polyline.str.extract(PatLat).values
Dat.loc[:,"Long"] = Dat.Polyline.str.extract(PatLong).values


# Tp= Dat.Polyline.str.split("(",expand =True)
# Tp2 = Tp[1].str.split("/", expand =True)
# Lat = Tp2[0]
# Long = Tp2[1].str.split(")",expand=True)[0]


# st ='(34.18571/ -78.08056)l(34.187870000000004/ -78.07909000000001)l(34.18824/ -78.0789)l(34.18862/ -78.07876)l'
# re.search('\((\d+.\d+)/',st).group(1)
# re.search('/(.-\d+.\d+)\)',st).group(1)


DatComb = pd.concat([Dat_ClMerDiv,Dat_LnAddDrp,Dat_Multi_RampTy])
writer = pd.ExcelWriter("NC-Processed-Data.xlsx")
