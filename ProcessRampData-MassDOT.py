...
def I495_RampVol(file,writer):
    ...
    Dat_AM.columns = ['Time','Tue','Wed','Thur','Fri','Sat','Sun','Mon','AvgDay']
    ...
    Dat_AM = Dat_AM.groupby(['Hr','Min'])[['Fri','Sat','Sun','Mon']].sum().reset_index()
    Dat_PM.columns = ['Time','Tue','Wed','Thur','Fri','Sat','Sun','Mon','AvgDay']
    ...
    Dat_PM = Dat_PM.groupby(['Hr','Min'])[['Fri','Sat','Sun','Mon']].sum().reset_index()
...
#    Dat_Fin[['Fri','Sat','Sun','Mon']] = Dat_Fin[['Fri','Sat','Sun','Mon']].div(Dat_Fin[['Fri','Sat','Sun','Mon']].sum(axis=0),axis =1)*100
    ...
    Dat_Fin.rename(columns = {'Fri':'Friday_{}-I495'.format(Ramp_Nm),'Sat':'Saturday_{}-I495'.format(Ramp_Nm),'Sun':'Sunday_{}-I495'.format(Ramp_Nm),'Mon':'Monday_{}-I495'.format(Ramp_Nm)},inplace=True)
    Dat_Fin = Dat_Fin[['Friday_{}-I495'.format(Ramp_Nm),'Saturday_{}-I495'.format(Ramp_Nm),'Sunday_{}-I495'.format(Ramp_Nm),'Monday_{}-I495'.format(Ramp_Nm)]] 
    ADT.rename(columns = {'Fri':'Friday_{}-I495'.format(Ramp_Nm),'Sat':'Saturday_{}-I495'.format(Ramp_Nm),'Sun':'Sunday_{}-I495'.format(Ramp_Nm),'Mon':'Monday_{}-I495'.format(Ramp_Nm)},inplace=True)
    startrow1 = 0
...
os.chdir(r'C:\Users\abibeka\OneDrive - Kittelson & Associates, Inc\Documents\MassDOT\Volumes\RampVolumes\I-495')
file ='175766 (17) Volume (Week).xls'
files = ['175766 (17) Volume (Week).xls',
