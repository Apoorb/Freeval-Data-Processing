...
def read_vol(file):
    ...
    Dat1 = Dat1[~((Dat1.index >= pd.datetime(2018,11,22)) & (Dat1.index < pd.datetime(2018,11,27)))]
   
    Dat_adt = Dat_adt[~((Dat_adt.index >= pd.datetime(2018,11,22)) & (Dat_adt.index < pd.datetime(2018,11,27)))]
...
def Process_Dat(Dat_Long, Dat_Adt):
    ...
    Dat2 = Dat_Long.groupby(['Day',Dat_Long.index.hour])['Vol'].sum()
    Dat2 = Dat2.swaplevel()
    Dat2 = Dat2.unstack()
    ...
    Dat3 = Dat2.div(Dat2.sum(axis=0), axis=1)*100
    # Check 
    Dat3.sum(axis=0)
...
for Counter in Counters:
    os.chdir(r'C:\Users\abibeka\OneDrive - Kittelson & Associates, Inc\Documents\MassDOT\Volumes\Main-Counters\{}'.format(Counter))
...
#    fig.savefig('{}_VolProfile.pdf'.format(Counter))
    ...
    ADT.to_excel(writer,'VolProfile',startrow = 0,startcol=0)
    VolProfile.to_excel(writer,'VolProfile',startrow =4 ,index=True)
    writer.save()
